...
namespace stockTable.Service
{
    public class SearchService
    {
        ...
        public IEnumerable<LowEquipment> GetEquipment(IEnumerable<LowEquipment> equipments, string searchField)
        {
            ...
            if (!String.IsNullOrEmpty(searchField) && equipments.Count() != 0)
            {
                ...
                var result = searchByModel.Union(searchByTypeEq).Union(searchByIp).Union(searchBySerialNum).Union(searchByResponsibly);
                return result;
            }
            else
            {
